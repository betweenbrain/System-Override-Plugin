...
class JDocumentRendererRSS extends JDocumentRenderer {
	...
	function render() {
		...
		$plugin    = JPluginHelper::getPlugin('system', 'overrides');
		$params    = new JParameter($plugin->params);
		$authorName = htmlspecialchars($params->get('authorName', ''));
		...
		for ($i = 0; $i < count($data->items); $i++) {
			...
			if ($authorName != "") {
				$feed .= "			<author>" . htmlspecialchars($data->items[$i]->authorEmail . ' (' .
					$authorName . ')', ENT_COMPAT, 'UTF-8') . "</author>\n";
			} elseif ($data->items[$i]->authorEmail != "") {
				$feed .= "			<author>" . htmlspecialchars($data->items[$i]->authorEmail . ' (' .
